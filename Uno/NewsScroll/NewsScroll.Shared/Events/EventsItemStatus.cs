        ...
        public static async Task<bool> ListViewScroller(ListView TargetListView, int CurrentItemId, int PreviousItemId)
        {
            try
            {
                ...
                if (TargetListViewItems > 0)
                {
                    ...
                    int MsgBoxResult = await new MessagePopup().OpenPopup("Item scroller", "Would you like to scroll in the items?", "Scroll to beginning", "Scroll to the middle", "Scroll to the end", "Scroll to unread item", ReturnToPrevious, true);
                    if (MsgBoxResult == 1)
                    {
                        await Task.Delay(10);
