    ...
    public partial class ApiPage : Page
    {
        ...
        private async Task RefreshFeeds()
        {
            try
            {
                ...
                int MsgBoxResult = await new MessagePopup().OpenPopup("Refresh feeds", "Do you want to refresh the feeds and scroll to the top?", "Refresh feeds", "", "", "", "", true);
                if (MsgBoxResult == 1)
                {
        ...
        private async Task AddFeedToApi()
        {
            try
            {
                ...
                if (!CheckLogin())
                {
                    await new MessagePopup().OpenPopup("Not logged in", "Adding a feed can only be done when you are logged in.", "Ok", "", "", "", "", false);
                    return;
                }
        ...
        async void btn_IgnoreFeeds_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    await new MessagePopup().OpenPopup("No feeds selected", "Please select some feeds that you want to un/ignore first.", "Ok", "", "", "", "", false);
                    return;
                }
                else
                {
                    try
                    {
                        ...
                        await new MessagePopup().OpenPopup("Feeds have been un/ignored", "Their items will be hidden or shown again on the next news item refresh.", "Ok", "", "", "", "", false);
                    }
                    catch
                    {
                        await new MessagePopup().OpenPopup("Failed to un/ignore feeds", "Please try to un/ignored the feeds again.", "Ok", "", "", "", "", false);
                    }
        ...
        async void btn_DeleteFeeds_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (ListView_Items.SelectedItems.Count == 0)
                {
                    await new MessagePopup().OpenPopup("No feeds selected", "Please select some feeds that you want to delete first.", "Ok", "", "", "", "", false);
                    return;
                }
                else
                {
                    ...
                    try
                    {
                        ...
                        await new MessagePopup().OpenPopup("Feeds have been deleted", "The feeds and it's items will disappear on the next refresh.", "Ok", "", "", "", "", false);
                    }
                    catch
                    {
                        await new MessagePopup().OpenPopup("Failed to delete feeds", "Please check your account settings, internet connection and try again.", "Ok", "", "", "", "", false);
                    }
