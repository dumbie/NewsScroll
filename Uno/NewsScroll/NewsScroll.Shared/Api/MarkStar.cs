    ...
    partial class Api
    {
        ...
        static public async Task<bool> MarkItemAsStarPrompt(Items ListItem, bool Confirm, bool RemoveFromList, bool ForceStar, bool Silent, bool EnableUI)
        {
            ...
            try
            {
                ...
                if (Confirm)
                {
                    int MsgBoxResult = await new MessagePopup().Popup(ActionType + " item", "Do you want to " + ActionType.ToLower() + " this item?", ActionType + " item", "", "", "", "", true);
            ...
            {
                await new MessagePopup().Popup("Failed to " + ActionType.ToLower() + " item", "Please check your internet connection and try again.", "Ok", "", "", "", "", false);
