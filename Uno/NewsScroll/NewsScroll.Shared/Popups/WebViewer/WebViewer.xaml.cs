    ...
    public partial class WebViewer : UserControl
    {
        ...
        private async Task GoBack()
        {
            try
            {
                if (webview_Browser.CanGoBack)
                {
                    ...
                    int MsgBoxResult = await AVMessageBox.Popup("Webviewer", "Do you want to go to the previous page or close the browser?", "Go to previous page", "Close webviewer", "", "", "", true);
        ...
        async void iconStar_Tap(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                string CurrentPageName = string.Empty;//FixApp.vApplicationFrame.SourcePageType.ToString();
        ...
        async void iconBrowser_Tap(object sender, RoutedEventArgs e)
        {
            ...
            {
                await HideShowMenu(true);
                int MsgBoxResult = await AVMessageBox.Popup("Open this item or link", "Do you want to open this item or link in your webbrowser?", "Open in webbrowser", "", "", "", "", true);
                if (MsgBoxResult == 1)
                {
                    ClosePopup();
